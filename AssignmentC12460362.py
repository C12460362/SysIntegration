...
print "Hello '%s' You are now using customised shell tsh" % uname
...
class Shell:
    ...
    def do_ifconfig(self, args):
        ...
        print ifconfig
        subprocess.call(["ifconfig eth0", "ifc"])


    def do_vi(self, args):
        m = re.match(r"((\S+)='([^']+)')?$", args)
        subprocess.call('vi', shell=True)
    ...
    def do_adduser(self, args):
        m = re.match(r"((\S+)='([^']+)')?$", args)
        subprocess.call('adduser', shell=True)
...
def repl():
    ...
    while True:
        ...
        try:
            line = raw_input(username + '/' + os.getcwd() + '/Custom Shell >>>')
            if line.strip() == 'exit':
                break
